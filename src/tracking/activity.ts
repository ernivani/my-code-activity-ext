...
export async function trackChanges(fileName: string) {
    try {
        ...
        console.log('File changed:', fileName, `(+${linesAdded}, -${linesRemoved})`);
    } catch (error) {
        vscode.window.showErrorMessage(`Failed to track changes for file: ${fileName}`);
        console.error('Error tracking changes:', error);
    }
}

async function getGitDiffStats(fileName: string): Promise<{ linesAdded: number; linesRemoved: number }> {
    try {
...
async function fallbackDiff(fileName: string): Promise<{ linesAdded: number; linesRemoved: number }> {
    try {
...
export async function ensureDailyDirectory(trackingRepoPath: string): Promise<string> {
...
export async function createActivityLog(trackingRepoPath: string): Promise<void> {
    ...
    try {
        ...
        for (const [projectName, projectStats] of Object.entries(mergedStats.projects)) {
            const projectPath = path.join(dayFolderPath, sanitizeFileName(projectName));
            await fs.promises.mkdir(projectPath, { recursive: true });
            await updateProjectReadme(projectPath, projectName, projectStats);
        }

        await fs.promises.writeFile(activityJsonPath, JSON.stringify(mergedStats, null, 2), 'utf-8');
...
function aggregateStats(): DailyStats {
    ...
    if (projectNames.length > 0) {
        ...
        const timePerProject = Math.floor(activeTimeInMinutes / projectNames.length);
...
function mergeDailyStats(oldStats: DailyStats, newStats: DailyStats): DailyStats {
    ...
    for (const [projName, newProjectStats] of Object.entries(newStats.projects)) {
        ...
        for (const [fileName, changesArr] of Object.entries(newProjectStats.files)) {
...
function isEmptyStats(stats: DailyStats): boolean {
    ...
    return projectList.every(
        (p) => p.totalLinesAdded === 0 && p.totalLinesRemoved === 0 && p.totalActiveTime === 0
    );
}
...
function generateMermaidCharts(projectStats: ProjectStats): string {
    ...
    for (const changesArray of Object.values(projectStats.files)) {
        for (const change of changesArray) {
            ...
            const timestamp = change.timestamp instanceof Date ? change.timestamp : new Date(change.timestamp);
...
async function updateProjectReadme(projectPath: string, projectName: string, stats: ProjectStats) {
...
export async function createHourlyLogs(projectPath: string, projectStats: ProjectStats) {
...
export async function createSummary(): Promise<string> {
    ...
    const totalAdded = Object.values(stats.projects).reduce((sum, p) => sum + p.totalLinesAdded, 0);
    const totalRemoved = Object.values(stats.projects).reduce((sum, p) => sum + p.totalLinesRemoved, 0);

    return `Changed ${trackedChanges.length} files across ${Object.keys(stats.projects).length} projects. ` +
