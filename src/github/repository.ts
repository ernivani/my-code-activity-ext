...
export async function ensureCodeTrackingRepo(
  token: string,
  username: string,
): Promise<string> {
...
`;
  ...
  const getReadmeResp = await fetch(
    `https://api.github.com/repos/${username}/code-tracking/contents/README.md`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: "application/vnd.github.v3+json",
  ...
  const updateReadmeResp = await fetch(
    `https://api.github.com/repos/${username}/code-tracking/contents/README.md`,
    {
      method: "PUT",
      headers: {
...
export async function ensureLocalRepo(
  ...
  try {
    ...
    // Configure git user
    await exec(
      `git -C ${localPath} config user.email || git -C ${localPath} config user.email "code-tracker@example.com"`,
    );
    await exec(
      `git -C ${localPath} config user.name || git -C ${localPath} config user.name "Code Tracker"`,
    );
...
export async function commitAndPush(
  ...
  try {
    ...
    await exec(
      `git -C ${localPath} remote add origin ${withAuth(remoteUrl, token)}`,
    );
