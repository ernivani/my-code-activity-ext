...
import { getGithubToken, signInToGitHub, tryGetExistingSession, fetchUserInfo } from './github/auth';
import { ensureCodeTrackingRepo, ensureLocalRepo, commitAndPush } from './github/repository';
import { trackChanges, createActivityLog, createSummary, ensureDailyDirectory } from './tracking/activity';
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    const signInCommand = vscode.commands.registerCommand('codeTracker.signInWithGitHub', async () => {
    ...
    if (storedToken) {
        ...
        try {
            ...
            if (userInfo.login) {
                outputChannel.appendLine(`Valid token found for user: ${userInfo.login}`);
                vscode.window.showInformationMessage(`Auto-signed in as ${userInfo.login}`);
                statusBar.update(true);
                await setupCodeTracking(context);
                return;
            }
        } catch (error) {
            outputChannel.appendLine('Stored token is invalid, will try other authentication methods');
        }
    }
    ...
    if (session) {
        outputChannel.appendLine(`Found existing session for user: ${session.account.label}`);
        vscode.window.showInformationMessage(`Auto-signed in as ${session.account.label}`);
        statusBar.update(true);
        await setupCodeTracking(context);
    } else {
        outputChannel.appendLine('No existing GitHub session found. Attempting automatic sign-in...');
        statusBar.update(false);
        if (await signInToGitHub()) {
            outputChannel.appendLine('Successfully signed in to GitHub');
            statusBar.update(true);
            ...
            vscode.window.showInformationMessage('Please sign in with GitHub for code tracking.', 'Sign In')
                .then((choice) => {
                    if (choice === 'Sign In') {
...
async function setupCodeTracking(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        outputChannel.appendLine(`Setting up local repository at ${Config.TRACKING_REPO_PATH}...`);
        await ensureLocalRepo(Config.TRACKING_REPO_PATH, REMOTE_REPO_HTTPS_URL, token);
        ...
        const timer = setInterval(async () => {
            ...
            if (currentToken && REMOTE_REPO_HTTPS_URL) {
                ...
                await commitAndPush(Config.TRACKING_REPO_PATH, summary, currentToken, REMOTE_REPO_HTTPS_URL);
            }
